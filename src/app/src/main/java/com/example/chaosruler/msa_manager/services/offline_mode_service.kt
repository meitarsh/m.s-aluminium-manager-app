...
//            context.startActivity(main_intent)
    ...
    private suspend fun db_sync_func_without_mark()
    {
        ...
        get_projid_to_sync_job.start()
        get_projid_to_sync_job.join()
        ...
        val load_db_job = async {
            load_dbs()
        }


         val projeccts_sync_job =async {
...
//                big_table.endTrans()
            ...
            val vend_table_job = async {
                global_variables_dataclass.log("db_sync", "vendor")
//                vendor.beginTrans()
                vendor.sync_db_by_key<vendor_data>(global_variables_dataclass.vendor_to_sync)
//                vendor.endTrans()
            ...
            val opr_table_job = async {
                global_variables_dataclass.log("db_sync", "opr")
//                opr.beginTrans()
                global_variables_dataclass.log("opr_sync", "Preparing to sync opr")

                opr.sync_db_by_key<opr_data>(global_variables_dataclass.opr_to_sync)
                global_variables_dataclass.log("opr_sync", "Opr synced ${global_variables_dataclass.DB_OPR!!.get_db()}")
//                opr.endTrans()
                global_variables_dataclass.log("db_sync", "opr done")

            }

            val salprojluz_sync_job = async {
...
//                salproj_table.endTrans()
            ...
            val takala_table_sync_job = async {
...
//                takala_table.endTrans()
     ...
     * @author Chaosruler972
     */
    private suspend fun load_dbs()
    {

        val db_proj_load = async {
            ...
            global_variables_dataclass.log("load", "DB BIG ${global_variables_dataclass.db_project_vec.size}")
        }

        val db_opr_load = async {
            global_variables_dataclass.log("load", "Started syncing opr")
            global_variables_dataclass.log("offline_mode", "Should load ${global_variables_dataclass.DB_OPR!!.get_db().size} Elements")
            global_variables_dataclass.db_opr_vec = opr.get_local_DB()
            global_variables_dataclass.log("load", "OPR ${global_variables_dataclass.db_opr_vec.size}")

        }

        val db_vendor_load = async {
...
//        db_big_load.start()
        global_variables_dataclass.log("offline_mode", "Starting threads")
        db_proj_load.start()
        db_opr_load.start()
        ...
        db_proj_load.join()
        global_variables_dataclass.log("offline_mode", "Proj done")

        db_opr_load.join()
        global_variables_dataclass.log("offline_mode", "opr done")

        db_vendor_load.join()
        global_variables_dataclass.log("offline_mode", "vendor done")

        db_salprojluz_load.join()
        global_variables_dataclass.log("offline_mode", "luz done")
